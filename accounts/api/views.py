...
def send_sms(phone_number, message):
    ...
    if response.status_code == 200:
        ...
        return response.json().get("message", "Failed to send SMS")
...
def resend_email_verification(request):
    ...
    _msg = f'Your BoodedNise OTP code is {otp_code}'
    url = f"https://apps.mnotify.net/smsapi"
    api_key = settings.MNOTIFY_KEY  # Replace with your actual API key
    print(api_key)
    response = requests.post(url,
...
@permission_classes([AllowAny])
@authentication_classes([])
def resend_shop_email_verification(request):
    payload = {}
    data = {}
    errors = {}
    email_errors = []
