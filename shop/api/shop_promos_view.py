...
from shop.api.serializers import AllShopPromotionsSerializer, ShopPromotionDetailsSerializer
from shop.models import Shop, ShopPackage, ShopPromotion
...
def add_shop_promotion(request):
    payload = {}
    data = {}
    errors = {}
...
@permission_classes([IsAuthenticated, ])
@authentication_classes([TokenAuthentication])
def get_all_shop_promotions_view(request):
    payload = {}
    data = {}
    errors = {}
    ...
    all_promotions = ShopPromotion.objects.all().order_by('-created_at')
    ...
    all_promotions_serializer = AllShopPromotionsSerializer(paginated_promotions, many=True)
...
def get_promotion_details_view(request):
    ...
    if not promotion_id:
        errors['promotion_id'] = ["ShopPromotion id required"]

    try:
        promotion = ShopPromotion.objects.get(promotion_id=promotion_id)
    except ShopPromotion.DoesNotExist:
        errors['promotion_id'] = ['ShopPromotion does not exist.']
    ...
    promotion_serializer = ShopPromotionDetailsSerializer(promotion, many=False)
...
def archive_promotion(request):
    ...
    if request.method == 'POST':
        ...
        if not promotion_id:
            errors['promotion_id'] = ['ShopPromotion ID is required.']
...
def get_all_archived_promotions_view(request):
    ...
    all_promotions = ShopPromotion.objects.all().filter(is_archived=True)
